...
async function getAdminSession() {
  ...
  if (session.user.role !== "admin") {
    throw new Error("Only administrators can access platform configuration.")
  }

  if (!session.user.id) {
    throw new Error("Administrator ID is missing from the session.")
  }
...
function validateMatchingWeights(weights: MatchingWeights) {
  const values = [
  ...
  for (const value of values) {
    if (typeof value !== "number" || !Number.isFinite(value)) {
      throw new Error("All matching weights must be valid numbers.")
    }

    if (value < 0 || value > 100) {
      throw new Error("Each matching weight must be between 0% and 100%.")
    }
  }
...
function serializePlatformConfig(
  ...
  return {
    ...
    matchingWeights: {
      availabilityWeight: config.matchingWeights.availabilityWeight,

      priceWeight: config.matchingWeights.priceWeight,

      supplierTypeWeight: config.matchingWeights.supplierTypeWeight,

      fulfillmentWeight: config.matchingWeights.fulfillmentWeight,

      reliabilityWeight: config.matchingWeights.reliabilityWeight,
    },

    minCreditApprovalLimit: config.minCreditApprovalLimit,

    maxCreditApprovalLimit: config.maxCreditApprovalLimit,

    autoAdvanceSupplierTimeoutSeconds: config.autoAdvanceSupplierTimeoutSeconds,
...
export async function updateMatchingWeights(weights: MatchingWeights): Promise<{
  success: true
  matchingWeights: MatchingWeights
}> {
