...
export default function MatchingAlgorithmWeightsTuner({
  ...
  const initialWeights: MatchingWeights = {
    availability: matchingWeights.availabilityWeight,

    priceCompetitiveness: matchingWeights.priceWeight,

    supplierType: matchingWeights.supplierTypeWeight,

    fulfillmentHistory: matchingWeights.fulfillmentWeight,

    reliability: matchingWeights.reliabilityWeight,
  }

  const [weights, setWeights] = useState<MatchingWeights>(initialWeights)
  ...
  const handleSaveWeights = async () => {
    ...
    try {
      ...
      const payload: PlatformMatchingWeights = {
        ...
        priceWeight: weights.priceCompetitiveness,

        supplierTypeWeight: weights.supplierType,

        fulfillmentWeight: weights.fulfillmentHistory,

        reliabilityWeight: weights.reliability,
      }

      const result = await updateMatchingWeights(payload)

      if (!result.success) {
        throw new Error("Failed to update matching weights.")
      }

      toast.success("Supplier matching weights deployed successfully.")
    } catch (error) {
      console.error("[MATCHING WEIGHTS] Failed to save:", error)
      ...
      <div className="mx-auto w-full max-w-7xl space-y-6">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Configure how the supplier matching engine scores, ranks, and
            prioritizes verified suppliers.
          </p>
        </div>
        ...
        <div className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
          ...
          <div className="flex flex-col justify-between gap-4 border-b border-slate-100 pb-4 sm:flex-row sm:items-center">
            <div>
              <h3 className="font-display flex items-center gap-2 text-base font-bold text-slate-900">
                ...
                <span>Multi-Factor Supplier Matching Scoring Model</span>
              </h3>

              <p className="mt-0.5 max-w-3xl text-xs text-slate-500">
                Calibrate how the engine scores and ranks verified suppliers
                when a hospital issues a procurement request.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 gap-6 text-xs md:grid-cols-2">
            ...
            <div className="space-y-2 rounded-xl border border-slate-100 bg-slate-50 p-4">
              <div className="flex justify-between gap-4 font-semibold text-slate-800">
                <span>1. Stock Availability &amp; MOQ Match</span>
              ...
              <input
                ...
                onChange={(e) =>
                  updateWeight("availability", Number(e.target.value))
                }
                className="h-2 w-full cursor-pointer appearance-none rounded-lg bg-slate-200 accent-blue-600"
              />

              <p className="text-[11px] text-slate-500">
                Penalizes low stock and rewards immediate fulfillment capacity.
              </p>
            </div>
            ...
            <div className="space-y-2 rounded-xl border border-slate-100 bg-slate-50 p-4">
              <div className="flex justify-between gap-4 font-semibold text-slate-800">
                <span>2. Price Competitiveness</span>
              ...
              <input
                ...
                onChange={(e) =>
                  updateWeight("priceCompetitiveness", Number(e.target.value))
                }
                className="h-2 w-full cursor-pointer appearance-none rounded-lg bg-slate-200 accent-blue-600"
              />

              <p className="text-[11px] text-slate-500">
                Compares unit price against the lowest available offer in the
                verified pool.
              </p>
            </div>
            ...
            <div className="space-y-2 rounded-xl border border-slate-100 bg-slate-50 p-4">
              <div className="flex justify-between gap-4 font-semibold text-slate-800">
                <span>3. Supplier Tier Hierarchy</span>
              ...
              <input
                ...
                onChange={(e) =>
                  updateWeight("supplierType", Number(e.target.value))
                }
                className="h-2 w-full cursor-pointer appearance-none rounded-lg bg-slate-200 accent-blue-600"
              />

              <p className="text-[11px] text-slate-500">
                Importer (100 pts) &gt; Distributor (70 pts) &gt; Retailer (40
                pts).
              </p>
            </div>
            ...
            <div className="space-y-2 rounded-xl border border-slate-100 bg-slate-50 p-4">
              <div className="flex justify-between gap-4 font-semibold text-slate-800">
                <span>4. Fulfillment Rate History</span>
              ...
              <input
                ...
                onChange={(e) =>
                  updateWeight("fulfillmentHistory", Number(e.target.value))
                }
                className="h-2 w-full cursor-pointer appearance-none rounded-lg bg-slate-200 accent-blue-600"
              />

              <p className="text-[11px] text-slate-500">
                Historical delivery on-time rate and low rejection count.
              </p>
            </div>
            ...
            <div className="space-y-2 rounded-xl border border-slate-100 bg-slate-50 p-4 md:col-span-2">
              <div className="flex justify-between gap-4 font-semibold text-slate-800">
                <span>5. Reliability &amp; Quality Rating</span>
              ...
              <input
                ...
                onChange={(e) =>
                  updateWeight("reliability", Number(e.target.value))
                }
                className="h-2 w-full cursor-pointer appearance-none rounded-lg bg-slate-200 accent-blue-600"
              />

              <p className="text-[11px] text-slate-500">
                Buyer star ratings and NAFDAC compliance audit scores.
              </p>
            </div>
          </div>
          ...
          {totalWeight !== 100 && (
            <div className="rounded-xl border border-red-100 bg-red-50 px-4 py-3 text-xs text-red-700">
              ...
              Adjust the scoring factors until the total allocation equals
              exactly 100%.
            </div>
          )}
          ...
          <div className="flex justify-end border-t border-slate-100 pt-4">
            ...
            >
              ...
              <span>
                {isSavingWeights ? "Saving..." : "Deploy Calibrated Weights"}
              </span>
            </button>
          </div>
        </div>
