...
export const MasterProductCatalog: React.FC<MasterProductCatalogProps> = ({
  products,
  inventory,
}) => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return [...catalogProducts]
      ...
      .sort((a, b) => {
        switch (sortBy) {
          case "price-asc":
            return Number(a.referenceBasePrice) - Number(b.referenceBasePrice)

          case "price-desc":
            return Number(b.referenceBasePrice) - Number(a.referenceBasePrice)
  ...
  }, [catalogProducts, searchQuery, selectedCategory, selectedStatus, sortBy])
  ...
  const handleExecuteDeleteOrArchive = async (archiveOnly: boolean) => {
    ...
    try {
      ...
      if (archiveOnly) {
        ...
        setCatalogProducts((current) =>
          current.filter((product) => product.id !== deleteConfirmProduct.id)
        )
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col items-start justify-between gap-4 lg:flex-row lg:items-center">
        <div>
          <h2 className="flex items-center gap-2 text-lg font-bold text-slate-900">
            <span>Master Pharmaceutical Product Catalog</span>
          </h2>

          <p className="mt-0.5 text-xs text-slate-500">
            Central NAFDAC-approved registry, standardized reference pricing,
            and automated platform commission controls.
          </p>
        </div>
      ...
      <CatalogStats products={catalogProducts} />
      ...
      {filteredProducts.length === 0 ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center">
          ...
          <h3 className="mt-3 text-sm font-bold text-slate-800">
            No matching pharmaceutical lines found
          </h3>
          ...
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-5 shadow-xl">
            ...
            <div className="mt-4">
              <h3 className="text-sm font-bold text-slate-900">
                Archive or Delist {deleteConfirmProduct.name}?
              </h3>

              <p className="mt-1 text-xs text-slate-500">
                Archiving retains the product record and audit history while
                preventing new hospital sourcing.
              </p>
            </div>
            ...
            <div className="mt-4 space-y-1 rounded-xl border border-slate-200 bg-slate-50 p-3 font-mono text-xs text-slate-600">
              <div>
                Reference Base: ₦
                {Number(deleteConfirmProduct.referenceBasePrice).toLocaleString(
                  "en-NG"
                )}
              </div>

              <div>Status: {deleteConfirmProduct.status}</div>
            </div>
            ...
            <div className="mt-4 space-y-2">
              <button
                ...
                onClick={() => handleExecuteDeleteOrArchive(true)}
              ...
              <button
                type="button"
                disabled={isSubmitting}
                onClick={() => handleExecuteDeleteOrArchive(false)}
