...
/* =========================================================
   MOCK CURRENT SUPPLIER
========================================================= */

const MOCK_CURRENT_USER: User = {
  id: 'usr-supplier-1',
  name: 'May & Baker Procurement Team',
  organization: 'May & Baker Nigeria Plc',
  supplierType: 'IMPORTER',
  supplierApprovalStatus: 'APPROVED',
  pcnPremisesLicense: 'PCN-IMP-2024-0012',
  nafdacGdpLicense: 'NAFDAC/GDP/2024/091',
...
const SupplierDashboard: React.FC = () => {
  ...
  const pendingRequests = useMemo(
    () =>
      procurements.filter(
        (p) =>
          p.currentSupplierId === currentUser.id &&
          (p.status === 'SUPPLIER_CONTACTED' ||
            p.status === 'NEXT_SUPPLIER_PENDING')
      ),
    [procurements, currentUser.id]
  );

  const supplierOrders = useMemo(
    () =>
      orders.filter(
        (order) => order.supplierId === currentUser.id
      ),
              ...
              <p className="text-xs text-slate-500 mt-1 flex flex-wrap items-center gap-3">
                <span>
                  ...
                  <strong className="text-slate-700">
                    {currentUser.pcnPremisesLicense}
                  </strong>
                </span>
                ...
                <span>
                  Settlement Bank:{' '}
                  <strong className="text-slate-700">
                    {currentUser.settlementBankName} (
                    {currentUser.settlementAccountNumber})
                  </strong>
                </span>
              </p>
            </div>
          ...
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-5 text-white shadow-sm">
            ...
            <div className="mt-4">
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                {currentUser.settlementAccountName}
              </p>
            </div>
