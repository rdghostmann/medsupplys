...
> = ({
  ...
  orders = [],
    ...
    const [procurements, setProcurements] =
      useState<IncomingProcurementRequest[]>(
        incomingProcurementRequests
      );
    ...
    const handleRespondRFQ = async (
      ...
      setTimeout(() => {
        if (response === 'ACCEPT') {
          const orderNumber = `MS-ORD-${Date.now()
            .toString()
            .slice(-4)}`;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="p-5 border-b border-slate-100 flex flex-col bg-slate-50/50">
              ...
              >
                ...
                <button
                  onClick={() => router.push('/supplier/order-requests')}
                  className="self-end text-xs font-semibold text-blue-600 hover:text-blue-700 flex items-center gap-1"
                >
                  View Full Queue
                  <ChevronRight className="w-3 h-3" />
                </button>
              ...
              {pendingRequests.length === 0 ? (
                ...
                <div className="divide-y divide-slate-100">
                  {pendingRequests.slice(0, 3).map((req) => (
                    <div
                      key={req.id}
                      className="p-5 cursor-progress hover:bg-slate-50/80 transition-colors"
                    >
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <div className="text-[11px] text-emerald-600 font-medium">
                            ...
                            <span className="hidden"> (
                              {req.paymentMethod.replace(/_/g, ' ')})</span>
                          </div>
                        </div>
                      </div>

                      <div className="hidden mt-3 pt-3 border-t border-slate-100  gap-3">
