...
const mockOrders: Order[] = [
...

  {
    ...
    expiryDate: "2029-07-31",
    condition: "Ambient",
    notes: "Pending supplier confirmation and availability validation.",
  },
]
...
export const FallBackQueueMonitor: React.FC<FallBackQueueMonitorProps> = ({
  queue,
  orders,
  hasQueue,
  loading = false,
        ...
        <div className="rounded-2xl border border-amber-200 bg-white p-5 shadow-xs">
          <div className="mb-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-display text-xs font-bold tracking-wider text-slate-900 uppercase **:md:text-sm">
                Live Procurement Sourcing ({activeProcurements.length})
              </h2>
            </div>
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-800">
                        {procurementStatusLabel(proc.status)}
                      </span>
                    </div>

                    <p className="mt-1 text-[10px] text-slate-600 md:text-xs">
                      Quantity:{" "}
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-2 text-[11px] text-slate-500">
                      <span>Currently Contacting:</span>
                      ...
                      {currentAttempt && (
                        <span className="hidden text-slate-400">
                          • {proc.attemptHistory.length} supplier attempt
                          {proc.attemptHistory.length !== 1 ? "s" : ""}
                        </span>
                      )}
                    </div>
                  </div>
