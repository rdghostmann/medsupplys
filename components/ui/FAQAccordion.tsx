...
export const FAQAccordion: React.FC = () => {
  ...
  const FAQS = [
    ...
    },
    {
      ...
      answer:
        "Every supplier on our platform undergoes a rigorous 4-step compliance audit: validation of national regulatory licenses (such as NAFDAC and Pharmacy Council operating permits), corporate registry authentication, verification of physical Good Distribution Practice (GDP) warehouse standards, and strict anti-counterfeiting history checks.",
    },
    {
      question:
        "What is the difference between Importer, Distributor, and Retailer tiers?",
      answer:
        "Importers bring WHO-prequalified and registered therapeutics into the country in bulk with higher Minimum Order Quantities (MOQ). Distributors maintain regional warehouses and offer intermediate pack sizes with 24-48 hour delivery. Retailers and institutional suppliers cater to smaller clinics requiring lower MOQs with same-day or rapid fulfillment.",
    },
    {
      question:
        "Can our hospital compare prices across multiple suppliers before ordering?",
      answer:
        "Yes. MedSupply provides instant side-by-side pricing matrices showing unit prices, pack sizes, volume-discount tiers, batch numbers, remaining shelf-life, and delivery lead times from every verified supplier carrying that SKU.",
    },
    {
      question:
        "How does MedSupply ensure cold-chain integrity during delivery?",
      answer:
        "Cold-chain products (such as insulin, biologicals, and vaccines) are fulfilled exclusively by GDP-certified distributors equipped with calibrated temperature-controlled active coolers and data loggers. Temperature compliance reports are verified at handover before goods receipt is signed.",
    },
  ]
        ...
        return (
          <div
            ...
            className={`overflow-hidden rounded-2xl border transition-all duration-200 ${
              isOpen
                ? "border-blue-600/40 bg-white shadow-sm"
                : "border-slate-200/90 bg-white hover:border-slate-300"
            }`}
          >
            <button
              onClick={() => toggle(idx)}
              className="flex w-full cursor-pointer items-center justify-between gap-4 px-6 py-4 text-left text-sm font-bold text-slate-900 focus:outline-hidden sm:text-base"
            ...
            {isOpen && (
              <div className="border-t border-slate-100 px-6 pt-1 pb-5 text-sm leading-relaxed text-slate-600">
                {faq.answer}
              </div>
            )}
          </div>
