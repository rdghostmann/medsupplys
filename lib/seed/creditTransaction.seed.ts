...
const BUYER_ID = new Types.ObjectId("6a9cb82d853e785e43c110b8")

const BUYER_NAME = "Lagos University Teaching Hospital (LUTH)"
...
export async function seedCreditTransaction() {
  ...
  if (!buyer) {
    throw new Error(`Buyer ${BUYER_ID.toString()} was not found.`)
  }

  if (buyer.role !== "buyer") {
    throw new Error(`User ${BUYER_ID.toString()} is not a buyer.`)
  }
  ...
  const creditTransaction = await CreditTransaction.findOneAndUpdate(
    ...
    },
    {
      $set: {
        ...
        reference: TRANSACTION_REFERENCE,
        ...(matchingOrder ? { orderId: matchingOrder._id } : {}),
        description:
          "Net-30 Credit Procurement for Artemether ACT Forte Course",
        createdAt: new Date("2025-01-11T11:00:00.000Z"),
      },
    },
    {
      upsert: true,
  ...
  if (!creditTransaction) {
    throw new Error("Failed to seed credit transaction.")
  }

  return {
    id: creditTransaction._id.toString(),
    creditAccountId: creditTransaction.creditAccountId.toString(),
