 ...
 */
const supplierUsernameMap: Record<string, string> = {
...
export async function seedSupplierProducts() {
  ...
  const productNameMap: Record<string, string> = {
    "prod-paracetamol-500": "Paracetamol 500mg Tablets",

    "prod-amoxicillin-500": "Amoxicillin 500mg Capsules",

    "prod-ibuprofen-400": "Ibuprofen 400mg Film-Coated Tablets",

    "prod-artemether-lumefantrine":
      "Artemether 80mg + Lumefantrine 480mg (ACT Forte)",

    "prod-vitamin-c-1000": "Vitamin C 1000mg Effervescent Tablets",

    "prod-metformin-500": "Metformin Hydrochloride 500mg Tablets",

    "prod-omeprazole-20": "Omeprazole 20mg Delayed-Release Capsules",

    "prod-cough-syrup": "Expectorant Cough Syrup with Menthol (100ml)",

    "prod-ors-sachets": "Oral Rehydration Salts (ORS) WHO Formula",

    "prod-amlodipine-5": "Amlodipine Besylate 5mg Tablets",
  }

  for (const seedId of productSeedIds) {
    const product = allProducts.find(
      (item) => item.name === productNameMap[seedId]
    )
   ...
   */
  const usernames = Object.values(supplierUsernameMap)
  ...
  const suppliers = await User.find({
    ...
    role: "supplier",
  }).select("_id username supplierType supplierApprovalStatus status")
  ...
  for (const [seedId, username] of Object.entries(supplierUsernameMap)) {
    const supplier = suppliers.find((item) => item.username === username)
  ...
  for (const item of supplierProducts) {
    try {
      const product = productIds.get(item.productId)

      const supplier = supplierIds.get(item.supplierId)

      if (!product) {
        throw new Error(`Product not found for seed ID: ${item.productId}`)
      }

      if (!supplier) {
        throw new Error(`Supplier not found for seed ID: ${item.supplierId}`)
      }
       ...
       */
      const supplierType = supplier.supplierType

      if (!supplierType) {
        throw new Error(`Supplier type missing for ${item.supplierId}`)
      }
       ...
       * Master Product.
       */
      const commissionPercent = product.commissionPercent

      const commission = Math.round(item.basePrice * (commissionPercent / 100))

      const finalPrice = item.basePrice + commission
      ...
      const supplierProductDocument = {
         ...
         * are available.
         */
        nafdacRegNumber: "SEED-NAFDAC-PENDING",
      ...
      if (existing) {
        await SupplierProduct.updateOne(
          { _id: existing._id },
          {
            $set: supplierProductDocument,
          }
        )

        results.updated.push(`${item.productId}:${item.supplierId}`)
      } else {
        await SupplierProduct.create(supplierProductDocument)

        results.created.push(`${item.productId}:${item.supplierId}`)
      }
    } catch (error) {
      results.failed.push({
